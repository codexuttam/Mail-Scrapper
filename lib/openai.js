...
export async function generateOutreach({ 
  ...
  let baseTemplate = "";
  if (campaignType === 'intro' && templateIntro) {
    baseTemplate = templateIntro;
  } else if (campaignType === 'offer' && templateOffer) {
    baseTemplate = templateOffer;
  } else if (campaignType === 'partnership' && templatePartnership) {
    baseTemplate = templatePartnership;
  }

  const prompt = `Write a ${tone} outreach message to a ${type} named ${name} in ${location}. 
  ${platformContext}
  ${campaignContext}
  ${baseTemplate ? `Please use this email template layout/copy as the base template. Replace placeholders like {{name}} and {{location}} with actual data, and polish the structure: "${baseTemplate}"` : ''}
  ${customPrompt ? `Follow this specific guideline/request for the outreach: ${customPrompt}` : ''}
  ...
  try {
    ...
    return "Error generating outreach message.";
  }
}
