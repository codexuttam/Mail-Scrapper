...
export async function scrapeBusinesses(query, options = {}) {
  ...
  for (const item of results) {
    ...
    try {
      ...
      // Extract coordinates from the URL if possible
      ...
      if (entry.website) {
        try {
          ...
          try {
            await sitePage.goto(entry.website, { waitUntil: 'domcontentloaded', timeout: 30000 });
          } catch (e) {
          ...
          let uniq = Array.from(new Set(combined.filter(e => typeof e === 'string' && e.includes('@')))).slice(0, 20);
