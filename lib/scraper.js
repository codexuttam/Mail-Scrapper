  ...
  for (let i = 0; i < results.length; i += batchSize) {
    ...
    const batchPromises = batch.map(async (item) => {
      ...
      try {
        ...
        // Validate phone number: must have at least 7 digits to avoid scraping garbage like "See photos"
        const digitCount = (info.phone || '').replace(/\D/g, '').length;
        entry.phone = digitCount >= 7 ? info.phone : '';
        ...
        if (entry.website) {
          try {
            ...
            if (combinedEmails.length === 0 && homeData.discoveryLinks.length > 0) {
              // Take top 3 unique discovery links to avoid infinite crawling but ensure coverage
              const linksToCrawl = Array.from(new Set(homeData.discoveryLinks)).slice(0, 3);
              
              for (const deepLink of linksToCrawl) {
                try {
                  await sitePage.goto(deepLink, { waitUntil: 'domcontentloaded', timeout: 12000 });
                  
                  // Aggressive scroll for lazy content on subpages
                  ...
                  // If we found emails, we can stop for this site
                  if (combinedEmails.some(e => e.includes('@'))) break;
                } catch (crawlErr) {
                  console.error(`Deep crawl failed for ${deepLink}:`, crawlErr.message);
                }
              }
            }
            ...
            entry.websiteText = homeData.txt;
            
            await sitePage.close();
          } catch (e) {
            console.error(`Site crawl failed for ${entry.website}:`, e.message);
          }
        }
      } catch (err) {
        console.error(`Lead enrichment failed for ${item.name}:`, err.message);
      } finally {
        if (p) await p.close();
      }
      return entry;
